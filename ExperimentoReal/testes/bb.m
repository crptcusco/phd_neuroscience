...
%    bb.m  March 16, 2019
%    EXPERIMENTO REAL
%    This program displays three diferents sounds for participants to make a choice response. In the blocks they press the numbers 1, 2, or 3 keys(keyboard - left arrow, down arrow, and right arrow).
...
line2 = '\n \n \n O objetivo deste é investigar os ní­veis de aquisição de habilidades';
...
line5 = '\n "Só passará para a próxima tentativa apóa acertar a tecla."';
...
for bloco = 1:5;
	WaitSecs(1.5);
	trial = 1+(bloco-1)*150:bloco*150;   %150 trials por bloco 
	while trial < length(seq)
    		Screen('FillOval', window, rectColor, centeredRect, maxDiameter);
    		Screen('Flip', window); %flipa a bolinha na tela
    		KbReleaseWait();
    		keyIsDown = 0;
...
save(baseName, 'vet', 'rvet', 'response', 'rt1', 'antecipatedresponse', 'avg', 'seq', 'responseTeste', 'rt2', 'avgTeste', 'antecipatedresponseT'); 
PsychPortAudio('Close');  
ListenChar(1);
ShowCursor;
